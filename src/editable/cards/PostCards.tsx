...
export function EditorialFeatureCard({ post, href, label = 'Cover story' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className="group block min-w-0 border-t-4 border-[var(--slot4-accent-fill)] bg-white text-black transition hover:bg-[#f4f4f4]">
      <div className="min-h-[320px] p-6 sm:p-9">
        <span className="bg-[var(--slot4-accent-fill)] px-3 py-2 text-[10px] font-black uppercase tracking-[.18em] text-white">{label}</span>
        <h3 className="mt-7 max-w-4xl text-4xl font-black leading-[1.02] tracking-[-.035em] sm:text-6xl">{post.title}</h3>
        <p className="mt-5 max-w-2xl text-sm leading-7 text-black/65 sm:text-base">{getEditableExcerpt(post, 220)}</p>
      </div>
    </Link>
  )
}
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block border-t-4 border-black bg-[var(--slot4-surface-bg)] ${dc.motion.lift}`}>
      <div className="p-5">
        ...
        <p className="mt-4 line-clamp-4 text-sm leading-6 text-black/60">{getEditableExcerpt(post, 145)}</p>
      </div>
    </Link>
  )
}
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className="group block min-w-0 border-t border-black/25 py-6">
      <div className="min-w-0">
