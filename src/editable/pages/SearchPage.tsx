...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`group block border-b border-r border-black bg-white transition hover:bg-[#f4f4f4] ${strong ? 'md:col-span-2' : ''}`}>
      <div className="p-5 sm:p-6">
        <span className="bg-[var(--slot4-accent-fill)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white">{taskLabel}</span>
        <h2 className="mt-5 line-clamp-3 text-2xl font-black leading-[1.05] tracking-[-0.025em] text-black">{post.title}</h2>
        <p className="mt-4 line-clamp-4 text-sm font-semibold leading-7 text-black/65">{summary || 'Details will appear here once available.'}</p>
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#e9e9e9] text-black">
        <section className="mx-auto max-w-[1140px] bg-[#e9e9e9]">
          <div className="grid border-b border-black bg-white md:grid-cols-[0.8fr_1.2fr]">
            <div>
              <div className="h-full bg-[var(--slot4-accent-fill)] p-7 text-white sm:p-10 lg:p-14">
            ...
            <form action="/search" className="self-center p-6 sm:p-10 lg:p-14">
              ...
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center border border-black bg-black px-6 text-xs font-black uppercase tracking-[0.18em] text-white transition hover:bg-[var(--slot4-accent-fill)]" type="submit">Search</button>
            </form>
          </div>
