...
function EditorialArchive({
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#e9e9e9] text-[#050505]">
        <section className="border-b border-black/25 bg-white">
          <div className="mx-auto flex max-w-[1140px] flex-col gap-6 px-4 py-12 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-0 lg:py-16">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent-fill)]">Media desk</p>
            ...
            <p className="editorial-serif max-w-xl text-xl leading-9 text-[#6d737b]">
              Timely reporting, public updates, and media-ready stories arranged for fast discovery across categories.
            </p>
          </div>
        </section>

        <section className="border-b border-black/25 bg-[#e5e5e5]">
          <div className="mx-auto flex max-w-[1140px] gap-0 overflow-x-auto px-4 py-4 text-sm font-black uppercase sm:px-6 lg:px-0">
            <Link href={basePath} className={category === 'all' ? 'px-4 text-[var(--slot4-accent-fill)]' : 'px-4 hover:text-[var(--slot4-accent-fill)]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'whitespace-nowrap border-l border-black px-4 text-[var(--slot4-accent-fill)]' : 'whitespace-nowrap border-l border-black px-4 hover:text-[var(--slot4-accent-fill)]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1140px] bg-white lg:grid-cols-[1.65fr_0.85fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden border-b border-black/20 lg:border-b-0 lg:border-r">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="bg-[var(--slot4-accent-fill)] px-3 py-2 text-[10px] font-black uppercase tracking-[0.2em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-[0.98] tracking-[-0.02em] sm:text-6xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid">
              <div className="border-b border-black/20 bg-[var(--slot4-accent-fill)] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="editorial-serif mt-3 text-3xl font-black leading-tight">What the media desk is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-b border-black/20 bg-white last:border-b-0">
        ...
        <section className="mx-auto max-w-[1140px] bg-[#e9e9e9] px-4 py-12 sm:px-6 lg:px-0 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-4 border-black pb-4">
            <h2 className="text-3xl font-black uppercase sm:text-4xl">More from the desk</h2>
          ...
          {remaining.length ? (
            <div className="grid gap-7 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className={index % 3 === 0 ? 'group bg-white shadow-sm' : index % 3 === 1 ? 'group grid bg-white shadow-sm sm:grid-rows-[auto_1fr]' : 'group border-t-4 border-[var(--slot4-accent-fill)] bg-white shadow-sm'}>
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            ...
            <span className="border-y border-black bg-[var(--slot4-accent-fill)] px-5 py-3 text-xs font-black uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
