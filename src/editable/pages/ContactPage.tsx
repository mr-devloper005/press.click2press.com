...
export default function ContactPage() {
  return (
    <EditableSiteShell>
      <main className="bg-[#e9e9e9] text-[#111]">
        <section className="border-b border-black bg-white">
          <div className="mx-auto max-w-[var(--editable-container)] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
            ...
            <p className="mt-6 max-w-2xl border-l-4 border-[var(--slot4-accent-fill)] pl-5 text-base font-semibold leading-8 text-black/65">{pagesContent.contact.description}</p>
          </div>
        </section>

        <section className="mx-auto grid max-w-[var(--editable-container)] border-x border-black bg-white lg:grid-cols-[0.72fr_1.28fr]">
          <aside className="border-b border-black bg-[#606160] text-white lg:border-b-0 lg:border-r">
            {desks.map((desk, index) => (
              <div key={desk.title} className="border-b border-white/25 p-7 last:border-b-0 sm:p-9">
                <div className="flex items-center justify-between"><desk.icon className="h-5 w-5 text-[var(--slot4-accent-fill)]" /><span className="text-xs font-black text-white/45">0{index + 1}</span></div>
